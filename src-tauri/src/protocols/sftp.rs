...
pub fn list(manager: &mut SshManager, session_id: &str, path: &str) -> Result<Vec<SftpEntry>> {
...
pub fn read(
    ...
    manager.sftp_read(session_id, path, chunk_size)
}
...
pub fn read_chunk(
    ...
    manager.sftp_read_chunk(session_id, path, offset, chunk_size)
}
...
pub fn write(
    ...
    manager.sftp_write(session_id, path, content, chunk_size)
}
